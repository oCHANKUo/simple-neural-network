...
# and sort of relates them to a value between 1 and 0. I imagine it as, a value 
# between 1 and 0 is representing the real value we used as the input.
...
# Output Dataset
y = np.array([[0,0,1,1]]).T # .T means transpose, resulting in a column vector. So, 0,0,1 output is 0, and so on.
...
for iter in range(10000): # Loop iterates 10000 times to train the nerwork

    # forward propagation
    l0 = x  # Input Layer 0 with data x
    l1 = nonlin(np.dot(l0, syn0))  # Output layer, np.dot() multiplies inputs by weights

    # how much did we miss?
    l1_error = y - l1  # Actual output y - predicted output.
